...
$app->get("/teams/:team/create", function ($team) use ($app) {
    $v = $app->validation;
    ...
    $app->response->headers->set("Cache-Control", "post-check=0, pre-check=0", false);
...
$app->get("/teams/:team/get", function ($team) use ($app) {
    $v = $app->validation;
...
$app->post("/teams/:team/set/:item", function ($team, $item) use ($app) {
    $v = $app->validation;
